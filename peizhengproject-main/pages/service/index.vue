		...
		<block v-if="service.stype == '50'">
			<view class="pub-box">
				<view class="pub-box-bd">
					...
					<view class="weui-cell weui-cell_input" @click="onAddressChange">
						<view class="weui-cell__hd">
							<view class="weui-label">上门信息</view>
						</view>
						<view class="weui-cell__bd"></view>
						<view class="weui-cell__ft weui-cell__ft_in-access">
							<input class="weui-input" :disabled="true" style="text-align: right"
								placeholder-class="vp-placeholder" placeholder="请选择上门信息" :value="userReceiveInfo" />
							<!-- {{order.address?(order.address.userName+'('+order.address.telNumber+')'):''}} -->
						</view>
					</view>
					<view class="weui-cell weui-cell_input">
			...
			<view class="pub-box">
				...
				<view class="pub-box-bd">
					<view class="weui-cell weui-cell_input">
						<view class="weui-cell__bd">
							<textarea name="demand" auto-height placeholder="请简单描述您的沐浴要求.."
								placeholder-class="vp-placeholder" style="min-height: 150rpx" v-model="order.demand" />
						</view>
					</view>
				</view>
...
<script setup>
	...
	const submit = () => {
		...
		const hospitalsData = toRaw(hospitals.value) // 返回没响应式的医院对象
	...
	const createOrder = (params) => {
		...
		app.globalData.utils.request2({
			...
			method: 'POST',
			  data: {
				params: {
				  ...params,  // 保留原有的params数据
				  trade_state: trade_state  // 将 trade_state 放入 params
				}
			  },
			success: res => {
				console.log("成功")
