...
onLoad(() => {
  ...
  app.globalData.utils.request2({
    url: '/Index/index',
    data: {
      aid: 1 // 假设 aid 是固定值或可以从其他地方获取
    },
    success: (res) => {
      console.log('请求成功，响应数据:', res);
      if (res && res.data && res.data.data) {
        bannerList.value = res.data.data.slides; // 轮播图
        nav2s.value = res.data.data.nav2s; // 两个导航
        navs.value = res.data.data.navs; // 多个导航
        hospitalList.value = res.data.data.hospitals; // 医院列表
        console.log(bannerList.value, 'res');
      } else {
        console.error('响应数据格式不正确:', res); // 调试语句：响应数据格式不正确
      }
    },
    fail: (error) => {
      console.error('请求失败:', error); // 调试语句：请求失败
    }
  });
});
	// 导航跳转
