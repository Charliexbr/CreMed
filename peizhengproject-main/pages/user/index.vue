	...
	<view>
			...
			<view style="margin-top: 40rpx">
				<view
					...
					<block v-if="mine.avatar">
						<image :src="mine.avatar" style="width: 150rpx; height: 150rpx" />
					</block>
					<block v-else>
		...
		<view class="weui-cells op-cells" style="margin-top: 20rpx">
			<view class="weui-cell weui-cell_access" hover-class="weui-cell_active" @tap="toServiceManager">
				...
				<view class="weui-cell__bd">服务人员选择</view>
		...
		<!-- 在这里添加浮动按钮 -->
		<l-fab axis="xy" magnetic="x" @change="onOffsetChange" @click="onClick">
		  <img src="@/static/robot_square.png" alt="Logo" class="fab-image">
		</l-fab>
		<!-- 手机登录弹窗 -->
...
<script setup>
	...
	const getUserInfo = () => {
		...
		app.globalData.utils.request2({
			...
			success: (res) => {
				console.log('Response:', res); // 打印返回的结果
				console.log('Data Type:', typeof res); // 打印数据类型
	...
	const countdownChange = () => {
		...
		app.globalData.utils.request2({
			...
			fail: res => {
				console.log('Error Response:', res); // 打印错误响应
        const errorMessage = res.msg || '发送验证码失败，请稍后重试。'; // 确保有默认消息
				uni.showToast({
					title: res.msg,
					duration: 1000,
					icon: 'none'
