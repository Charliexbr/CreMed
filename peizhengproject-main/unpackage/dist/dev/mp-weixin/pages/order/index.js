...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const onCancel = (id, tradeState) => {
      console.log(`取消订单: ${id}, 状态: ${tradeState}`);
      if (tradeState == 1 || tradeState == 2) {
        console.log("符合条件，可以取消订单");
        app.globalData.utils.request2({
          url: "/Order/cancel",
          method: "POST",
          data: {
        ...
        console.log("不符合条件，无法取消订单");
      }
    };
    const onDelete = (id, tradeState) => {
      console.log(`删除订单: ${id}, 状态: ${tradeState}`);
      app.globalData.utils.request2({
        url: "/Order/delete",
        method: "POST",
        data: {
      ...
      return common_vendor.e({
        ...
        n: common_vendor.f(list.value, (item, index, i0) => {
          return common_vendor.e({
            ...
            r: common_vendor.o(($event) => onCancel(item.id, item.trade_state), index),
            s: common_vendor.o(($event) => onDelete(item.id, item.trade_state), index),
            t: item.trade_state == "待支付"
          }, item.trade_state == "待支付" ? {
            v: common_vendor.o(onCounterOver, index),
            w: "22939a92-3-" + i0,
            x: common_vendor.p({
              second: item._exp_time
            })
          } : {}, {
            y: item.trade_state == "待服务"
          }, item.trade_state == "待服务" ? {} : {}, {
            z: item.trade_state == "已完成"
          }, item.trade_state == "已完成" ? {} : {}, {
            A: item.trade_state == "已取消"
        ...
        x: common_vendor.sr(popup, "22939a92-4", {
          "k": "popup"
        }),
        y: common_vendor.p({
          type: "center",
